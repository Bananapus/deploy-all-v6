...
**This repo's deviations:** `evm_version = 'cancun'`, `via_ir = true`, `optimizer = false` (disabled — full ecosystem import causes stack-too-deep with optimization). This is a deployment-only repo with no `src/` directory. Has `[lint] lint_on_build = false`. Package name: @bananapus/deploy-all-v6
