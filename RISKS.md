...
4. **Deployment Ordering** -- Contracts deploy in dependency order within a single `deploy()` call. Between Sphinx proposal submission and execution, no intermediate state is unsafe because contracts are not usable until the full wiring phase completes. If a proposal is only partially executed, the recovery path is to bump the deployment nonce and redeploy the full stack from fresh salts.
...
|------|----------|-------------|------------|
| Partial deployment | HIGH | If Sphinx execution halts mid-proposal (gas, revert, platform failure), core contracts may exist without wiring. Example: `JBDirectory` deployed but `setIsAllowedToSetFirstController` never called -- no projects can launch. | Do not re-propose the full script with the same salts; CREATE2 reverts on the already-deployed contracts. Bump the deployment nonce in `Deploy.s.sol` so every salt re-namespaces into a fresh, non-colliding address space, redeploy the full stack, then run `script/Verify.s.sol` to validate state and separately verify ownership/admin addresses. |
| Fresh-salt redeploy identity mismatch | HIGH | A fresh-salt redeploy must be proposed from the same Safe identity used by the original path. Using the wrong Safe address in env/config produces a deployment owned by the wrong authority for that chain. | Treat the deployment Safe as part of the trusted deployment config, redeploy from the same Safe identity, and verify ownership/admin endpoints after recovery. |
| Controller deploys after omnichain deployer | MEDIUM | `JBController` constructor takes `omnichainRulesetOperator: address(_omnichainDeployer)`. If the omnichain deployer address changes between phases, the controller is permanently misconfigured. | Single-proposal atomicity ensures deterministic ordering. `_omnichainDeployer` is deployed via `_deployCreate3PrecompiledIfNeeded`, which now verifies `addr.codehash` against a sandbox-CREATE reference codehash on both the early-return and post-deploy paths — an attacker who pre-deploys at the predicted CREATE3 address (via the permissionless canonical factory) is rejected before the controller can be baked with the wrong immutable. Still verify the `_omnichainDeployer` address and runtime bytecode in the controller's constructor after deployment. |
...
**Re-proposing the full script with the same salts is not a safe recovery path** because CREATE2 with the same salt
and initcode will revert once any matching deployment already exists. Bump the deployment nonce in `Deploy.s.sol` so
every salt re-namespaces into a fresh address space, redeploy the full stack, then run `script/Verify.s.sol` to
...
| DEFIFA start-time anchor persistence | HIGH | DEFIFA is the only not-yet-launched revnet; its start time is encoded into the revnet configuration hash and therefore the sucker salt derivation. The exact timestamp used by the accepted Sphinx proposal must be reused by post-deploy address dumping and verification, even if the timestamp has passed by then. Always propose through `pnpm deploy:propose:*`, which pins and caches `DEFIFA_REV_START_TIME` under `script/post-deploy/.cache/`. |
| DEFIFA stale execution window | HIGH | Sphinx proposal collection runs `Deploy.s.sol`, but the later Safe transaction calls runtime contracts directly. If signers execute after the pinned DEFIFA start timestamp, `REVDeployer.deployFor` can still queue stage 0 as already active and add a cash-out delay. The proposal helper now defaults to a seven-day lead and both the helper and deploy script reject new DEFIFA configs inside a three-day lead window; stale proposals should be cancelled and re-proposed. |
| Banny drop partial execution | HIGH | Banny drop registration is a follow-up Sphinx proposal with separate tier and resolver metadata writes. If a proposal adds tiers and then stops before metadata writes, a naive tier-count idempotency check would strand products without SVG/name metadata. Drop reruns now re-check committed SVG hashes, write only missing hashes, and rerun product names before final ownership/operator handoff. |
| Live smoke loan permission mutation | MEDIUM | The live loan smoke can require granting REVLoans `BURN_TOKENS` permission for the deployment Safe before opening and repaying a test loan. If execution is interrupted after the grant but before restoration, the permission could remain. Loan smoke is now disabled by default and permission mutation requires `SMOKE_ALLOW_PERMISSION_MUTATION=true`; monitor any opted-in loan smoke through repayment and permission restoration. |
...
| Risk | Severity | Description |
|------|----------|-------------|
| Sphinx Safe compromise | HIGH | The V6 deployment Safe can execute the deployment proposal and one-shot chain-specific setup. Persistent critical owner roles currently converge to the same V6 Deployment Safe (`0x4dc161eF837fF1C4485b08DDFcDB182F2157bE18`), which is also the NANA revnet operator. |
...
3. **Bump the deployment nonce in `Deploy.s.sol`.** This is the single salt-version lever for the entire deployment: every CREATE2/CREATE3 salt is folded with the nonce, so incrementing it yields a fully fresh, non-colliding address namespace for every contract. The mined Uniswap V4 hook salt re-derives over the new namespace as well.
4. **Redeploy the full stack from the same deployment Safe.** Because the addresses are fresh, the redeploy runs from a clean slate with no half-deployed contracts to reconcile, and converges to the same Safe-owned boundary as the canonical path.
