...
contract DeployForkTest is Test {
    ...
    function test_deployFork_fullInfrastructure() public {
        vm.startPrank(_deployer);
        ...
        // Core contracts non-zero.
        assertTrue(address(_controller) != address(0), "Controller not deployed");
        ...
        assertTrue(
            ...
            "Controller not allowed to set first controller"
        );

        // 721 Hook wiring.
        assertTrue(address(_hookStore) != address(0), "HookStore not deployed");
        assertTrue(address(_hookDeployer) != address(0), "HookDeployer not deployed");
        ...
        // Omnichain Deployer exists.
        assertTrue(address(_omnichainDeployer) != address(0), "OmnichainDeployer not deployed");

        // Price feeds: USD/ETH feed returns a reasonable price.
        ...
        assertTrue(pricePerUnit > 100e18, "ETH price too low");
        assertTrue(pricePerUnit < 100_000e18, "ETH price too high");
    }

    /// @notice Verifies that the Deploy script contract has non-empty bytecode (compilation check).
    /// This catches any compilation errors in the deploy script's imports and constructor logic.
    function test_deployFork_scriptCompilation() public pure {
        // If this test compiles and runs, all imports from Deploy.s.sol are valid.
